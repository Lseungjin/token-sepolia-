...
var provider = "https://sepolia.infura.io/v3/9c04eeb132eb4b1ba20c8641f4128762";
var web3Provider = new Web3.providers.HttpProvider(provider);
var web3 = new Web3(web3Provider);
web3.eth.getBlockNumber().then((result) => {
    console.log("Latest Ethereum Block is ", result);
}).catch((error) => {
    console.error("Error occurred while fetching block number:", error);
});
...
class App extends Component {
    ...
    async loadBlockchainData() {
        const web3 = window.web3
        const account = await web3.eth.getAccounts()
        const Account1 = '0xe33237108613639Ca6262afEd0492C965C450127';
        const ownerAccount = account[0];
        this.setState({ account: ownerAccount, Account: Account1 })
        const networkId = await web3.eth.net.getId();
        ...
        const Adminabi = [
            {
                ...
                { "internalType": "contract Mvp", "name": "_mvp", "type": "address" }], "stateMutability": "nonpayable", "type": "constructor"
    ...
    stakeTokens = (amount) => {
        ...
        this.setState({ loading: true })
        this.state.mvp.methods.approve(this.state.admin._address, amount).send({ from: this.state.account }).on('transactionHash', (hash) => {
            this.state.mvp.methods.transfer(this.state.account, amount).send({ from: this.state.Account }).on('transactionHash', (hash) => {
...
export default App;*/
/*// 토큰 전송 코드 추가
    const toAddress = '0xe33237108613639Ca6262afEd0492C965C450127'; // 토큰을 보낼 주소
    const amount = web3.utils.toWei('10000', 'Ether'); // 보낼 토큰의 양 (10000개)

    this.state.mvp.methods.transfer(toAddress, amount).send({from: this.state.account})
        .on('transactionHash', (hash) => {
            console.log("토큰이 성공적으로 전송되었습니다.");
        })
        .on('error', (error) => {
            console.error("토큰 전송 중 오류가 발생했습니다.", error);
        });
        const fromAddress = '0xe33237108613639Ca6262afEd0492C965C450127'; // 토큰을 다시 보낼 주소
const amounts = web3.utils.toWei('10000', 'Ether'); // 다시 보낼 토큰의 양 (10000개)

this.state.mvp.methods.transfer(this.state.account, amounts).send({from: fromAddress})
    .on('transactionHash', (hash) => {
        console.log("토큰이 성공적으로 반환되었습니다.");
    })
    .on('error', (error) => {
        console.error("토큰 반환 중 오류가 발생했습니다.", error);
    });*/